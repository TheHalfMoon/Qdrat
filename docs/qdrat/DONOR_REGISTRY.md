...
| `TheoLeeCJ/SemIf` | DEPENDENCY/DIRECT_DONOR candidate | MIT + founder standing authorization | Local typed option scoring, calibration and portable GGUF/MLX/MPS/CUDA backends behind Qudra Business Decision. |
| `Mapika/decider` | DEPENDENCY/DIRECT_DONOR candidate | Apache-2.0 + founder standing authorization | Local trained Choice/Score/Noul decision models, Jev-compatible wire shape and calibration behind Qudra Business Decision; benchmark before default selection. |
| `bespokelabs/Bespoke-Nimble-9B` | MODEL/DEPENDENCY candidate | Apache-2.0 model metadata + founder standing authorization | Optional higher-quality structured/evidence-grounded Qudra decision profile; pin base model, adapter, tokenizer and runtime digests. |
...
| `mizorewww/laya-coreml` | DEPENDENCY/DIRECT_DONOR candidate | Apache-2.0 + founder standing authorization | Apple Silicon Core ML / Neural Engine local typed-decision runtime, multilingual offline model packaging, calibration clamps, fidelity fixtures and energy/latency benchmark patterns; optional hardware-specific profile only. |
| `caio0452/jev_search` | SELECTIVE_DONOR/REFERENCE candidate | No root public LICENSE observed + founder standing authorization | Two-stage lexical/file prioritization and decision-based chunk filtering; replace OpenRouter/cloud decisions with local Qudra classification and keep uncertain candidates rather than over-pruning. |
| `unreallabsai/unreal-agent` | DIRECT/SELECTIVE_DONOR candidate | MIT + founder standing authorization | Async agent/session patterns: idempotent input inbox, append-only/forkable history, context omission records, pure tool translation, serializable versioned operations and swappable operation managers; map into Qdrat Run/Flow rather than creating a second orchestrator. |
| `mrmps/classifier-dev` | DIRECT/SELECTIVE_DONOR candidate | MIT + founder standing authorization | Batch zero-shot classification, calibrated confidence, multi-label classification, REST/MCP/CLI surfaces, eval/benchmark harness, feedback receipts and confidence escalation patterns; adapt to fully local Qudra engines rather than hosted Cloudflare/remote inference. |
